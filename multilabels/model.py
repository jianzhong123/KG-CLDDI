...
class KGCL(BasicModel):
    def __init__(self, config: dict, dataset: BasicDataset, kg_dataset):
        ...
        transE_entity_path = '/home/zhongjian/Val-KGCL-SIGIR22-main/data/TwoSides/DRKG_TransE_l2_entity.npy'#_300
        ...
        transE_r_path = '/home/zhongjian/Val-KGCL-SIGIR22-main/data/TwoSides/DRKG_TransE_l2_relation.npy'
        ...
        self.embedding_relation.weight.requires_grad =False#True#

        self.embedding_entity = nn.Embedding.from_pretrained(self.transemb_entity)
        self.embedding_entity.weight.requires_grad = False#True#
        ...
        self.layer1 = nn.Sequential(nn.Linear(600, 2048), nn.BatchNorm1d(2048), nn.ReLU(True))#768nn.Dropout(0.5),
        self.layer2 = nn.Sequential(nn.Linear(2048, 200))
        self.layer3 = nn.Sequential(nn.Linear(2048, 81))
        self.sigmoid = torch.nn.Sigmoid()

        self.fcl = nn.Sequential(nn.Linear(300, 150), nn.Dropout(0.1), nn.ReLU(True))#nn.BatchNorm1d(300),
        self.activation = nn.LeakyReLU()
        self.linear1 = nn.Linear(100, 100)  # self.latent_dim, self.latent_dim
        self.linear2 = nn.Linear(100, 100)  # W2 in Equation (8)

        self.linear11 = nn.Linear(100, 100)  # W1 in Equation (8)
        self.linear22 = nn.Linear(100, 100)  # W2 in Equation (8)
        '''
        df_drugs_smiles = pd.read_csv('data/processdata/drugname_smiles.csv')
        ...
        # 生成分子指纹矩阵
        fp_matrix = []
        for id, smiles in df_drugs_smiles.values:
            mol = Chem.MolFromSmiles(smiles)
            fp = AllChem.GetMorganFingerprintAsBitVect(mol, fp_radius, fp_length)
            fp_array = np.zeros((1,))
            DataStructs.ConvertToNumpyArray(fp, fp_array)
            fp_matrix.append(fp_array)  # 只保留前100维
        fp_matrix = np.vstack(fp_matrix)

        # 归一化处理
        #fp_matrix = fp_matrix / np.linalg.norm(fp_matrix, axis=1)[:, np.newaxis]
        #pca = PCA(n_components=100)
        #fp_matrix = pca.fit_transform(fp_matrix)
        self.mol=torch.tensor(fp_matrix).to(world.device).float()
        self.mol = nn.Embedding.from_pretrained(self.mol)
        self.mol.weight.requires_grad = True

        self.lstm = nn.LSTM(1, 128, bidirectional=True)
    ...
    def __dropout_x(self, x, keep_prob):
        ...
        return g
    '''
    def __dropout_x(self, all,x, keep_prob):

        values = x.values()
        index = x.indices().t()

        all_index=all.indices().t()
        all_values=all.values()
        size = all.size()
        train_index = torch.ones(len(all_values)).to(world.device)
        train_index[len(index):]=0
        #random_index = train_index.int().bool()
        dropnum = int((1 - keep_prob) * len(values))
        ran = torch.randperm(len(values)).numpy()
        ...
        #     random_index[idx]=0
        train_index[drops] = 0
        random_index = train_index.bool()
        all_index = all_index[random_index]
        all_values = all_values[random_index]  # /keep_prob#bigger
        # print(len(values))
        g = torch.sparse.FloatTensor(all_index.t(), all_values, size)
        return g
    '''
    def __dropout(self, keep_prob):
        #S1
        #graph = self.__dropout_x(self.AllGraph,self.Graph, keep_prob)
        graph = self.__dropout_x(self.Graph, keep_prob)

        return graph

    def computer(self,drop_prob=0.0):
        """
        ...
        for layer in range(self.n_layers):#3
            ...
            sum_embeddings = self.activation(self.linear11(all_emb + side_embeddings))
            bi_embeddings = self.activation(self.linear22(all_emb * side_embeddings))
    ...
    def computerAdj(self,DDIone,DDItwo,drop_prob=0.0):
        """
        propagate methods for lightGCN
        """
        items_emb = self.cal_item_embedding_from_kg(self.kg_dict)#self.embedding_item.weight#
        DDIone_emb = items_emb[DDIone.long()]
        DDItwo_emb = items_emb[DDItwo.long()]
        all_emb = torch.cat([DDIone_emb, DDItwo_emb])

        embs = []#all_emb
        embs1 = []
        #A_in = self.dropoedge(self.A_in,drop_prob)
        for layer in range(self.n_layers):#3
            #print(self.A_in.shape, all_emb.shape)
            side_embeddings = torch.matmul(self.Adj, all_emb)
            side_embeddings = F.dropout(side_embeddings, 0.1, training=self.training)
            sum_embeddings = self.activation(self.linear11(all_emb + side_embeddings))
            bi_embeddings = self.activation(self.linear22(all_emb * side_embeddings))
            embeddings = bi_embeddings + sum_embeddings
        ...
        embs = torch.stack(embs, dim=1)  # [95594, 4, 64]
        light_out = torch.mean(embs, dim=1)  # [95594, 64]
        users, items = torch.split(light_out,[DDIone_emb.shape[0], DDIone_emb.shape[0]])  # //users:[70679, 64],items:[24915, 64]
        embs1 = torch.stack(embs1, dim=1)  # [95594, 4, 64]
        light_out1 = torch.mean(embs1, dim=1)  # [95594, 64]
        users1, items1 = torch.split(light_out1,[DDIone_emb.shape[0], DDIone_emb.shape[0]])  # //users:[70679, 64],items:[24915, 64]
        return users, items, users1, items1, items_emb

    def computer1(self,DDIone,DDItwo,drop_prob=0.0):
        """
        propagate methods for lightGCN
        """
        items_emb = self.cal_item_embedding_from_kg(self.kg_dict)#self.embedding_item.weight#
        ...
        embs = torch.stack(embs, dim=1)#[95594, 4, 64]
        light_out = torch.mean(embs, dim=1)#[95594, 64]
        users, items = torch.split(light_out, [DDIone_emb.shape[0], DDIone_emb.shape[0]])

        #items_emb = F.normalize(items_emb, p=2, dim=1)
        return users,items,items_emb

    def getDDIEmbedding(self,DDIone,DDItwo,drop_prob=0.95):
        ...
        #DDIone_emb = self.embedding_entity.weight[DDIone.long()]  # all_entitis
        #DDItwo_emb = self.embedding_entity.weight[DDItwo.long()]
        ...
        # DDIone_struct=self.molvec[DDIone.long()]
        # DDItwo_struct = self.molvec[DDItwo.long()]
        # DDIone_struct=self.fcl(DDIone_struct)
        # DDItwo_struct=self.fcl(DDItwo_struct)
    ...
    def branch1_forward(self, DDIone_conv,DDIone_entitis,DDIone_emb, DDItwo_conv, DDItwo_entitis,DDItwo_emb,drop=0.1):  #
        ...
        result = self.layer2(inner_pro)
        result = self.sigmoid(result)
        return result

    def branch2_forward(self, Labelemb1,DDIone_emb, DDIone_items, DDIone_conv, DDItwo_emb, DDItwo_items, DDItwo_conv,drop=0.1):  #
        labelemb = self.fcl(Labelemb1)
        inner_pro = torch.cat([labelemb,DDIone_emb, DDIone_items, DDIone_conv, DDItwo_emb, DDItwo_items, DDItwo_conv],dim=1)
        inner_pro = self.layer1(inner_pro)
        result = self.layer2(inner_pro)
        result = self.sigmoid(result)
        return result
