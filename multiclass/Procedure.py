...
def sup_compute_loss(y_pred,lamda=0.7):#0.6
...
def Fun_bpr_loss(users_emb, pos_emb, neg_emb):

    pos_scores = torch.sum(users_emb * pos_emb, dim=1)
    neg_scores = torch.sum(users_emb * neg_emb, dim=1)
    #print("oos",pos_scores.shape)
    loss = torch.sum(torch.nn.functional.softplus(neg_scores - pos_scores))#mean
...
def DDItrain(recommend_model,optimizer,train_X, train_Y,scheduler):
    Recmodel: model.KGCL = recommend_model
    Recmodel.train()


    #traindf = pandas.read_csv("../data/processdata/DDITrainOrgin0.txt", delimiter=' ', header=None)
    #S1
    #traindf = pandas.read_csv("../data/processdata/MUltiClass_TrainAll_S1.csv", delimiter=',', header=None)
    #data = traindf.values
    ...
    for data, data3 in zip(loader_train, loader_train2):
        ...
        DDIone_conv, DDIone_entitis, DDIone_emb, DDItwo_conv, DDItwo_entitis, DDItwo_emb,neg_conv,neg_entitis,neg_emb = Recmodel.getDDIEmbedding(users, items,items3,0.95)
        out1 = Recmodel(DDIone_conv, DDIone_entitis, DDIone_emb, DDItwo_conv, DDItwo_entitis, DDItwo_emb, 0.1, mode='branch1')

        out1 = out1.squeeze(-1)
        #conout1 = conout1.squeeze(-1)
        DDIone = torch.cat([DDIone_conv, DDIone_entitis,DDIone_emb], dim=1)
        DDItwo = torch.cat([DDItwo_conv, DDItwo_entitis, DDItwo_emb], dim=1)
        DDIneg = torch.cat([neg_conv,neg_entitis,neg_emb], dim=1)
        bpr_loss =Fun_bpr_loss(DDIone,DDItwo,DDIneg)


        DDIone_conv2, DDIone_entitis2, DDIone_emb2, DDItwo_conv2, DDItwo_entitis2, DDItwo_emb2,neg_conv,neg_entitis,neg_emb = Recmodel.getDDIEmbedding(users, items,items3,0.95)
        #out = Recmodel(DDItwo_emb2, DDItwo_entitis2, DDItwo_conv2, DDIone_emb2, DDIone_entitis2, DDIone_conv2,0.4, mode='branch1')
        out = Recmodel(DDItwo_conv2, DDItwo_entitis2, DDItwo_emb2, DDIone_conv2, DDIone_entitis2, DDIone_emb2, 0.2,mode='branch1')
        out = out.squeeze(-1)
        #conout = conout.squeeze(-1)

        # DDIone = torch.cat([DDItwo_conv2, DDItwo_entitis2, DDItwo_emb2], dim=1)
        # DDItwo = torch.cat([DDIone_conv2, DDIone_entitis2, DDIone_emb2], dim=1)
        # DDIneg = torch.cat([neg_conv,neg_entitis,neg_emb], dim=1)
        # bpr_loss1 = Fun_bpr_loss(DDIone, DDItwo, DDIneg)

        DDIone_conv3, DDIone_entitis3, DDIone_emb3, DDItwo_conv3, DDItwo_entitis3, DDItwo_emb3 = Recmodel.getDDIEmbedding1(users3, items3, 1)
        out3 = Recmodel(DDIone_conv3, DDIone_entitis3, DDIone_emb3, DDItwo_conv3, DDItwo_entitis3, DDItwo_emb3, 0.1, mode='branch1')
        out3 = out3.squeeze(-1)
        #conout3 = conout.squeeze(-1)

        mul_loss = criterion(out, batch_label.long())
        mul_loss1 = criterion(out1, batch_label.long())
        ...
        loss=(mul_loss+mul_loss1)+0.1*con_loss#+0.1*bpr_loss#+0.1*bpr_loss#
...
def DDITest_Bin_MultiLabels(Recmodel,epoch,test_X, test_Y,):
    data = test_X
    ...
    for epoch,data in enumerate(loader_test):
        with torch.no_grad():
            ...
            DDIone_conv, DDIone_entitis, DDIone_emb, DDItwo_conv, DDItwo_entitis, DDItwo_emb = Recmodel.getDDIEmbedding1(drugs1, drugs2, 1)
            out1 = Recmodel(DDIone_conv, DDIone_entitis, DDIone_emb, DDItwo_conv, DDItwo_entitis, DDItwo_emb, 0.0,mode='branch1')
            out1 = out1.squeeze(-1)
            #DDIone_conv2, DDIone_entitis2, DDIone_emb2, DDItwo_conv2, DDItwo_entitis2, DDItwo_emb2  = Recmodel.getDDIEmbedding1(drugs1, drugs2, 1)
            #out = Recmodel(DDItwo_emb, DDItwo_entitis, DDItwo_conv, DDIone_emb, DDIone_entitis, DDIone_conv, 0.0,mode='branch1')
            ...
            result=0.5*out+0.5*out1
            results = np.concatenate((results, result.data.cpu().numpy()), axis=0)
            prediction = copy.deepcopy(result)
    ...
    if world.multi_labels == True:
        ...
        return out1s,outs,results,macro_f1
